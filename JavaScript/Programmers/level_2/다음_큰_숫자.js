...
function checkOneNumber(nBinaryArray, nextBinaryArray) {
    const oneNumberOfNBinaryArray = nBinaryArray.reduce((accumulator, currentValue) => {
        if (currentValue) return accumulator + 1;
        return accumulator + 0;
    }, 0);

    const oneNumberOfNextBinaryArray = nextBinaryArray.reduce((accumulator, currentValue) => {
        if (currentValue) return accumulator + 1;
        return accumulator + 0;
    }, 0);
